...
func run(ctx context.Context, config app.Config) error {
	...
	dlq := config.Kafka.Topic + ".dlq"
	consumer := consume.NewConsumer(
		client,
		// serde,
		dlq,
		consume.WithMessageHandler(handleRecord),
	)

	slog.InfoContext(ctx, "starting dlq consume")
	for {
...
func handleRecord(ctx context.Context, msg *dlqv1beta1.Record) error {
	// json, err := protojson.Marshal(invoice)
	// if err != nil {
	// 	return err
	// }

	slog.Info("got a dlq record")
